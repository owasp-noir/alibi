...
def build(index: Index, findings: list[Finding], skipped: list[Skipped],
          sources: list[str], ruleset: RuleSet) -> dict:
        ...
        "runs": [
            {
                ...
                "invocations": [
                    {
                        # Noir ran and the rules were evaluated. A rule held
                        # back is a conclusion, not a failure to reach one.
...
def dump(index: Index, findings: list[Finding], skipped: list[Skipped],
         sources: list[str], ruleset: RuleSet) -> str:
    return json.dumps(build(index, findings, skipped, sources, ruleset), indent=2)
...
def _result(finding: Finding, position: dict[str, int]) -> dict:
    entry = finding.entry
